...
Poor’s (S&P) stock market index. The years range from **1969 to 2068**,
...
contains **1056 observations** of **11 variables**. The data set
compiled all previously mentioned variables from the past three data
sets and matched them to month and year. Considering the `year`
variable, the final data set ranges from **1947 to 2068**. Since I
joined the three data sets using `full_join`, R automatically filled
missing data with `NA`. With this data set, we are able to compare the
number of Democrat vs. Republic governors, senators, and
representatives, the political party of the president, the stock closing
value, and unemployment rates in each month-year pair.
